...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[start:], start=1):
            msg += f"𝐅𝐢𝐥𝐞𝐍𝐚𝐦𝐞: <code>{download.name()}</code>"
            ...
            if download.status() not in [
                ...
                if download.status() == MirrorStatus.STATUS_CLONING:
                    msg += f"\n𝐂𝐥𝐨𝐧𝐞𝐝: <code>{get_readable_file_size(download.processed_bytes())}</code> of <code>{download.size()}</code>"
                elif download.status() == MirrorStatus.STATUS_UPLOADING:
                    msg += f"\n𝐔𝐩𝐥𝐨𝐚𝐝𝐞𝐝: <code>{get_readable_file_size(download.processed_bytes())}</code> of <code>{download.size()}</code>"
                else:
                    msg += f"\n𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝𝐞𝐝: <code>{get_readable_file_size(download.processed_bytes())}</code> of <code>{download.size()}</code>"
                msg += f"\n𝐒𝐩𝐞𝐞𝐝: <code>{download.speed()}</code> <b>ETA:</b> <code>{download.eta()}</code>"
                try:
                    msg += f"\n𝐒𝐞𝐞𝐝𝐞𝐫𝐬: <code>{download.aria_download().num_seeders}</code>" \
                           f" | 𝐏𝐞𝐞𝐫𝐬: <code>{download.aria_download().connections}</code>"
                except:
                    pass
                try:
                    msg += f"\n𝐒𝐞𝐞𝐝𝐞𝐫𝐬: <code>{download.torrent_info().num_seeds}</code>" \
                           f" | 𝐋𝐞𝐞𝐜𝐡𝐞𝐫𝐬: <code>{download.torrent_info().num_leechs}</code>"
                except:
                    pass
        ...
        if STATUS_LIMIT is not None and dick_no > STATUS_LIMIT:
            msg += f"𝐏𝐚𝐠𝐞: <code>{PAGE_NO}</code>/<code>{pages}</code> | 𝐓𝐚𝐬𝐤𝐬: <code>{dick_no}</code>\n"
