...
def stats(update, context):
    ...
    stats = f'<b>𝐁𝐨𝐭 𝐔𝐩𝐭𝐢𝐦𝐞:</b> <code>{currentTime}</code>\n' \
            f'<b>𝐓𝐨𝐭𝐚𝐥 𝐃𝐢𝐬𝐤 𝐒𝐩𝐚𝐜𝐞:</b> <code>{total}</code>\n' \
            f'<b>𝐔𝐬𝐞𝐝:</b> <code>{used}</code> ' \
            f'<b>𝐅𝐫𝐞𝐞:</b> <code>{free}</code>\n\n' \
            f'<b>𝐔𝐩𝐥𝐨𝐚𝐝:</b> <code>{sent}</code>\n' \
            f'<b>𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝:</b> <code>{recv}</code>\n\n' \
            f'<b>𝐂𝐏𝐔:</b> <code>{cpuUsage}%</code> ' \
            f'<b>𝐑𝐀𝐌:</b> <code>{memory}%</code> ' \
            f'<b>𝐃𝐢𝐬𝐤:</b> <code>{disk}%</code>'
    sendMessage(stats, context.bot, update)


def start(update, context):
    buttons = button_build.ButtonMaker()
    buttons.buildbutton("𝐓𝐞𝐚𝐦 𝐃𝐫𝐢𝐯𝐞", "https://groups.google.com/g/MoviesEmpire-td")
    buttons.buildbutton("𝐂𝐡𝐚𝐧𝐧𝐞𝐥𝐬", "https://t.me/TheEmpireGroup/4")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''
𝕵𝖔𝖎𝖓 𝕺𝖚𝖗 𝕲𝖗𝖔𝖚𝖕 <b>@TellyChat</b>
'''
        sendMarkup(start_string, context.bot, update, reply_markup)
    else:
        sendMarkup(
            '𝐎𝐩𝐩𝐬! 𝐧𝐨𝐭 𝐚 𝐀𝐮𝐭𝐡𝐫𝐢𝐳𝐞𝐝 𝐮𝐬𝐞𝐫.\n𝐏𝐥𝐞𝐚𝐬𝐞 𝐉𝐨𝐢𝐧 @TellyChat.',
            context.bot,
            update,
            reply_markup,
        )
...
help = Telegraph(access_token=telegraph_token).create_page(
        title='𝐌𝐨𝐯𝐢𝐞𝐬𝐄𝐦𝐩𝐢𝐫𝐞 𝐌𝐢𝐫𝐫𝐨𝐫𝐁𝐨𝐭 𝐇𝐞𝐥𝐩',
        author_name='𝐌𝐨𝐯𝐢𝐞𝐬𝐄𝐦𝐩𝐢𝐫𝐞 𝐌𝐢𝐫𝐫𝐨𝐫𝐛𝐨𝐭',
